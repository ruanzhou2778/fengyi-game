        ...
        if random.random() < 0.15 and npc.get("rank") not in ["皇后", "皇贵妃"]:
            ...
            for _ in range(child_count):
                ...
                if "children" not in npc:
                    npc["children"] = []
                npc["children"].append({"child_id": uuid.uuid4().hex, "name": child_name, "gender": gender, "age": age, "birth_day": random.randint(1,30), "birth_month": random.randint(1,12), "birth_year": 1, "trait": "🎀 襁褓" if age < 0.5 else ("🎂 周岁" if age < 1.5 else ("👶 幼童" if age < 3 else "🎓 启蒙")), "alive": True, "adopted_count": 0, "adopted": False, "birth_mother": npc["name"], "adoptive_mother": ""})
