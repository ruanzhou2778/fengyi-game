...
with m.app.test_client() as c:
 ...
 r=post(c,'/api/next_period',{'player_id':pid}); nd=r.get_json() or {}; top=nd.get('intrigue',{}).get('top_dirt',[]); ck('next_period',r.status_code==200,r.status_code); ck('day+10',nd.get('day')==day+10,f'{day}->{nd.get("day")}'); ck('intrigue exists',isinstance(nd.get('intrigue'),dict),nd.get('intrigue')); ck('heat<=25',nd.get('intrigue',{}).get('heat',999)<=25,nd.get('intrigue')); ck('dirt<=3',(top[0].get('points',99) if top else 99)<=3,top)
